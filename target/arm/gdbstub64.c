...
static int aarch64_elx_gdb_get_reg(CPUState *cs, GByteArray *buf, int reg, int el)
{
    ARMCPU *cpu = ARM_CPU(cs);
    CPUARMState *env = &cpu->env;
...
static int aarch64_elx_gdb_set_reg(CPUState *cs, uint8_t *buf, int reg, int el)
{
    ARMCPU *cpu = ARM_CPU(cs);
    CPUARMState *env = &cpu->env;
